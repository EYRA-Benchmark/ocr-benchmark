...
def processReadingorder(gtXML, inXML):
    ...
    gtOrderedGroup = sorted(gtXML.PcGts.Page.ReadingOrder.OrderedGroup.RegionRefIndexed, key=lambda RegionRefIndexed: RegionRefIndexed['index'])
    inOrderedGroup = sorted(inXML.PcGts.Page.ReadingOrder.OrderedGroup.RegionRefIndexed, key=lambda RegionRefIndexed: RegionRefIndexed['index'])
...
def processBoundingboxes(gtXML, inXML):
    ...
    gtBounds_copy, inBounds_copy, report = matchPolygonsAndRemove(gtBounds, inBounds)
    ...
    for in_id_a, in_box_a in inBounds.items():
        for in_id_b, in_box_b in inBounds.items():
            if in_id_a != in_id_b:
                ...
                if abs(maxx_b - minx_a) < PIXEL_MATCH_THRESHOLD and abs(miny_b - miny_a) < PIXEL_MATCH_THRESHOLD and abs(maxy_b - maxy_a) < PIXEL_MATCH_THRESHOLD:
                ...
                elif abs(maxy_b - miny_a) < PIXEL_MATCH_THRESHOLD and abs(minx_b - minx_a) < PIXEL_MATCH_THRESHOLD and abs(maxx_b - maxx_a) < PIXEL_MATCH_THRESHOLD:
                    inBounds_copy['union: '+in_id_a+','+in_id_b] = in_box_a.union(in_box_b)
                    del inBounds_copy[in_id_a]
                    del inBounds_copy[in_id_b]
                    print('created combined boundingbox {newid}'.format(newid=in_id_a+in_id_b))

    gtBounds_rest, inBounds_rest, report = matchPolygonsAndRemove(gtBounds_copy, inBounds_copy)
    end_report = end_report + report
...
def matchPolygonsAndRemove(gtBounds, inBounds):
    ...
    for gt_id, gt_box in gtBounds.items():
        ...
        if scores[max_index] > JACCARD_SCORE_THRESHOLD:
            ...
            report = report + '"{gt_id}" matched with "{in_id}" with score {score}\n'.format(in_id=match_id, gt_id=gt_id, score=scores[max_index])
    
    return (gtBounds_copy, inBounds_copy, report)
...
def processLayout(gtXML, inXML):
    ...
    for gt_id, gt_box in gtLayout.items():
        ...
        if scores[max_index] > JACCARD_SCORE_THRESHOLD:
            ...
            print('"{gt_id}" matched with "{in_id}" with score {score}'.format(in_id=match_id, gt_id=gt_id, score=scores[max_index]))
